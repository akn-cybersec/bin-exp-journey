...
A 32-bit binary with no win function, no libc leak, but a buffer overflow. We need to build a full ROP chain to write `/bin/sh` into memory and call `execve` via `int 0x80`.
